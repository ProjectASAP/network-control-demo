...
def capacity_constraints(optimizer, resources, bandwidth, allocation):
    ...
    for resource in resources:
        total_usage = sum(allocation[task] * resource.requirement for task in optimizer.tasks)
    ...
    for edge in bandwidth.edges:
        total_bandwidth = sum(allocation[task] * bandwidth.requirement[task] for task in optimizer.tasks if task in edge.tasks)
