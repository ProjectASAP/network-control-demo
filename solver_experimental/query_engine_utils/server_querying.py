...
from .config import QueryManagerConfig, QueryGroupConfig, ServerType, ServerConfig, QueryResult
    ...
    def _process_response(self, data: dict, query: str) -> QueryResult:
        ...
        if result_type == 'vector':
            for metric in result:
                task_id = metric['metric'][task_label] if task_label in metric['metric'] else None
    ...
    def _process_response(self, data: dict, query: dict) -> QueryResult:
        ...
        for agg_name, agg_data in aggregations.items():
            if 'buckets' in agg_data:
                ...
                buckets.append(QueryResult.Bucket(task_id=None, value=agg_data.get('value')))
