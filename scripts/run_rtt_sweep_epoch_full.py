...
DEFAULT_SERVER_LOG = "logs/server_epoch_full.log"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Epoch-based RTT sweep: ingest + query + solver timing for server and ES."
    )
    ...
    parser.set_defaults(
        ...
        out_csv="data/rtt_results_epoch_full.csv",
    )
    parser.add_argument(
        "--out-plot",
        type=str,
        default="plots/rtt_epoch_full.png",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run the Python solver once per epoch after queries",
    )
    parser.add_argument(
        "--solver-task-count",
        type=int,
...
# ---------------------------------------------------------------------------
# Solver helpers (copied from run_rtt_sweep_epoch_with_solver.py)
# ---------------------------------------------------------------------------
...
def run_solver_for_usage(
    ...
    topology = assets["NetworkTopology"](nodes.values(), context["edges"].values())
...
def plot_results(results: List[SweepResult], out_path: Path) -> None:
    ...
    ax.set_title("Per-epoch timing breakdown: Server vs ES\n(ingest + query + solver)")
    ...
    ax2.set_title("Total epoch time: Server vs ES\n(ingest + query + solver)")
    ax2.legend()
    ...
    try:
        ...
        for epoch in range(args.start_epoch, args.end_epoch + 1):
            ...
            if args.run_solver and assets is not None and solver_context is not None:
                print("  running solver on server metrics ...")
                ...
                print("  running solver on ES metrics ...")
