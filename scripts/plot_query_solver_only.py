...
def plot_query_solver(csv_path: Path, out_path: Path, backend: str) -> None:
    ...
    ax.bar(x - bar_w / 2, s_solver, bar_w, bottom=s_query,
           label="Solver (Approximate Input)", color="#59a14f")

    ax.bar(x + bar_w / 2, e_query,  bar_w, label="Elastic Search Query",  color="#f28e2b")
    ax.bar(x + bar_w / 2, e_solver, bar_w, bottom=e_query,
           label="Solver (Elastic Search Input)", color="#b07aa1")
...
def plot_query_solver_split(csv_path: Path, out_path: Path, backend: str) -> None:
    ...
    ax_s.bar(x - bar_w / 2, s_solver, bar_w, label="Solver (Approximate Input)", color="#59a14f")
    ax_s.bar(x + bar_w / 2, e_solver, bar_w, label="Solver (Elastic Search Input)", color="#b07aa1")
...
def plot_solver_only(csv_path: Path, out_path: Path, backend: str) -> None:
    """Single bar chart comparing solver time only (no query), linear y-axis."""
    rows = load_csv(csv_path)
    ...
    fig, ax_s = plt.subplots(figsize=(7.2, 4.4))
...
def plot_ingestion_only(csv_path: Path, out_path: Path, backend: str) -> None:
    """Single bar chart comparing ingestion time only, linear y-axis."""
    rows = load_csv(csv_path)
    ...
    epochs = [int(r["epoch"]) for r in rows]
    s_ingest = [float(r["server_ingest_ms"]) for r in rows]
    e_ingest = [float(r["es_ingest_ms"]) for r in rows]

    x = np.arange(len(epochs))
    bar_w = 0.35

    fig, ax_i = plt.subplots(figsize=(7.2, 4.4))

    ax_i.bar(x - bar_w / 2, s_ingest, bar_w, label="Approximate Ingestion", color="#4e79a7")
    ax_i.bar(x + bar_w / 2, e_ingest, bar_w, label="Elastic Search Ingestion", color="#e15759")
    ax_i.set_xlabel("Epoch", fontsize=LABEL_FS)
    ax_i.set_ylabel("Ingestion Time (ms)", fontsize=LABEL_FS)
    ax_i.grid(axis="y", alpha=0.3)
    ax_i.tick_params(axis="both", labelsize=TICK_FS)
    ax_i.set_title(
    ...
    ax_i.set_xticks(x)
    ax_i.set_xticklabels([str(e) for e in epochs])
...
def plot_ingest_query_solver_split(csv_path: Path, out_path: Path, backend: str) -> None:
    """Three stacked subplots (ingestion / query / solver) per epoch."""
    rows = load_csv(csv_path)
    ...
    fig, (ax_i, ax_q, ax_s) = plt.subplots(3, 1, figsize=(7.2, 9.0), sharex=True)
    ...
    ax_i.tick_params(axis="both", labelsize=TICK_FS)
    ax_i.set_title(
        "Ingestion+Query+Solver Time Comparison\n"
        "Approximate VS Exact",
        fontsize=TITLE_FS,
        pad=14,
    )
    ...
    # Combined legend at the bottom: all six series (ncol=2 -> approx/ES columns).
    handles = []
    labels = []
    for ax in (ax_i, ax_q, ax_s):
...
def plot_query_solver_logy(csv_path: Path, out_path: Path, backend: str) -> None:
    ...
    ax.bar(x - 1.8 * bar_w, s_solver, bar_w, label="Solver (Approximate Input)", color="#59a14f")
    ax.bar(x - 0.6 * bar_w, s_query, bar_w, label="Approximate Query", color="#2a9d8f")
    ax.bar(x + 0.6 * bar_w, e_solver, bar_w, label="Solver (Elastic Search Input)", color="#b07aa1")
    ax.bar(x + 1.8 * bar_w, e_query, bar_w, label="Elastic Search Query", color="#f28e2b")
...
if __name__ == "__main__":
    ...
    plot_solver_only(
        data_dir / "rtt_results_epoch_full_ortools_cbc_30nodes.csv",
        plot_dir / "rtt_epoch_solver_only_ortools_cbc_30nodes.png",
        "CBC",
    )
    plot_ingestion_only(
        data_dir / "rtt_results_epoch_full_ortools_cbc_30nodes.csv",
        plot_dir / "rtt_epoch_ingestion_only_ortools_cbc_30nodes.png",
        "CBC",
    )
    plot_ingest_query_solver_split(
        data_dir / "rtt_results_epoch_full_ortools_cbc_30nodes.csv",
        plot_dir / "rtt_epoch_ingest_query_solver_split_ortools_cbc_30nodes.png",
        "CBC",
    )
