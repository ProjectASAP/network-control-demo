...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="SCIP",
        help="OR-Tools backend to request (default: SCIP)",
    )
    parser.add_argument(
        "--max-epochs",
        type=int,
...
def _load_solver_assets(data_dir: Path) -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to import solver modules. Ensure solver_experimental dependencies are installed."
        ) from exc
...
def run_solver_for_usage(
    ...
    edges = context["edges"]
    tasks_list = [context["tasks"][task_id] for task_id in task_ids]

    solver = assets["NetworkControllerSolver"](
        nodes, edges, solver_backend=solver_backend
    )

    t0 = time.perf_counter()
    result = solver.solve(tasks_list, time_limit_s=time_limit_s)
    elapsed_ms = (time.perf_counter() - t0) * 1000.0

    assigned_task_ids = sorted(result.decisions.keys())
    unassigned_task_ids = sorted(result.unassigned_tasks)
    return SolverResult(
...
def plot_results(results: List[SweepResult], out_path: Path, backend: str = "SCIP") -> None:
    ...
    ax.set_title(f"Per-epoch timing breakdown: Server vs ES (OR-Tools/{backend} solver, emulator)")
...
def main() -> None:
    ...
    print(
        ...
        f"tasks={len(context['tasks'])}/{len(assets['tasks'])}, backend={args.solver_backend}"
    )
    ...
    try:
        ...
        for epoch in range(args.max_epochs):
            ...
            print(
                f"  solver start ({args.solver_backend}, limit={args.solver_time_limit_s:.1f}s): "
            ...
            print(
                f"  solver done ({args.solver_backend}, limit={args.solver_time_limit_s:.1f}s): "
                f"server={server_solver_ms:.2f} ms, ES={es_solver_ms:.2f} ms | "
                f"assigned(server/es)={server_sr.assignments}/{es_sr.assignments}"
            )

            # Build per-epoch new assignments from Sketch-path solver output.
