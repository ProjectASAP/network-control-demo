...
impl MultiHeadHydra {
    pub fn dimension_index(&self, dimension: &str) -> Option<usize> {
        self.dimensions.iter().position(|(name, _)| name == dimension)
    }
    ...
    pub fn update(&mut self, key: &str, values: &[(&SketchInput, &[&str])], count: Option<i32>) {
        ...
        let dim_name_to_idx: HashMap<&str, usize> = self
            ...
            .collect();
        let precomputed: Vec<(u128, Vec<usize>)> = values
            .iter()
            .map(|(value, dims)| {
        ...
        for i in 1..(1 << n) {
            ...
            self.sketches.fast_insert(
                |cell_vec, dim_values, _| {
                    ...
                    for ((value, _), (value_hash, indices)) in
                        values.iter().zip(precomputed.iter())
                    {
    ...
    pub fn merge(&mut self, other: &MultiHeadHydra) -> Result<(), String> {
        ...
        for (idx, (name, counter)) in self.dimensions.iter().enumerate() {
            let (other_name, other_counter) = other
                .dimensions
                .get(idx)
                .ok_or_else(|| "MultiHeadHydra dimension list mismatch while merging".to_string())?;
            if name != other_name {
                return Err(format!(
