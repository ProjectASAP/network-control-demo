...
Based on current documentation, the table below summarizes sketch-level optimizations.

| Sketch | Current optimizations (documented) | In-progress optimizations |
| --- | --- | --- |
| CMS | FastPath single-hash mode; `_with_hash_value` pre-hash insertion/estimation; Fixed-Size sketch with monomorphization for loop unrolling | TBD |
| CS | FastPath single-hash mode; `_with_hash_value` pre-hash insertion/estimation; Fixed-Size sketch with monomorphization for loop unrolling; L2-heavy-hitter variant (CountL2HH) | TBD |
| HLL | HllDf/HllDs variants; HllDs provides O(1) estimate with fastest query path | Fixed Size Bucket List |
| KLL | Memory compaction with pre-allocated memory; Cached level-size/height calculation | TBD |
| UnivMon | Count Sketch optimized for L2 heavy hitters (CountL2HH); TopK Heap optimization with HashMap to record element in heap | TBD |
| Hydra | Re-use Common API | TBD |
