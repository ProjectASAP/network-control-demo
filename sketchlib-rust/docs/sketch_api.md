...
All sketches in this library are **✅ RECOMMENDED** - they are optimized, actively maintained, and ready for production use.
...
- `Vector2D<i32>`: dynamic dimensions, `HashValue = u128`
- `FixedMatrix`: fixed 5 × 2048, `HashValue = u64`
...
fn insert(&mut self, value: &SketchInput)
fn estimate(&self, value: &SketchInput) -> i32
fn fast_insert_with_hash_value(&mut self, hashed_val: S::HashValue)
fn fast_estimate_with_hash(&self, hashed_val: S::HashValue) -> i32
```

**Nitro (Vector2D + FastPath):**

```rust
fn enable_nitro(&mut self, sampling_rate: f64)
fn disable_nitro(&mut self)
fn fast_insert_nitro(&mut self, value: &SketchInput)
fn nitro_estimate(&self, value: &SketchInput) -> f64
```
...
fn estimate(&self, value: &SketchInput) -> f64
fn fast_insert_with_hash_value(&mut self, hashed_val: S::HashValue)
fn fast_estimate_with_hash(&self, hashed_val: S::HashValue) -> f64
```
...
```rust
use sketchlib_rust::{CountMin, FastPath, Vector2D, SketchInput, hash_it_to_128};

let mut cm = CountMin::<Vector2D<i32>, FastPath>::default();
let key = SketchInput::U64(12345);
let hash = hash_it_to_128(0, &key);
...
```rust
use sketchlib_rust::{HllDf, SketchInput};
...
- **Cardinality:** HyperLogLog (3 variants: HyperLogLog, HllDf, HllDs)
- **Quantile:** KLL, DDSketch
- **Frameworks:** Hydra, UnivMon, NitroBatch
