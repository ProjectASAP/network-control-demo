...
use sketchlib_rust::common::{
    ...
    // Hash functions
    hash_it, hash_it_to_64, hash_it_to_128,
    hash_item_to_64, hash_item_to_128,
    hash_for_enough_bits,

    // Constants
    SEEDLIST, LASTSTATE, BOTTOM_LAYER_FINDER, HYDRA_SEED,
...
Wrapper for different sketch types with unified API.
...
pub trait MatrixStorage<T: Clone> {
    ...
    fn fast_query_min<F, R>(&self, hashed_val: Self::HashValue, op: F) -> R
        where F: Fn(&T, usize, Self::HashValue) -> R, R: Ord;
    fn fast_query_median<F>(&self, hashed_val: Self::HashValue, op: F) -> f64
        where F: Fn(&T, usize, Self::HashValue) -> f64;
...
- `Vector2D<i32>`: flexible dimensions, `HashValue = u128`
...
dimensions. Implements `MatrixStorage<i32>` with `HashValue = u64`.
...
Hashing uses XxHash3 for 64/128-bit paths, and XxHash32 when `hash_for_enough_bits` is asked for 32-bit output.
...
pub fn hash_item_to_64(d: usize, key: &HeapItem) -> u64
pub fn hash_item_to_128(d: usize, key: &HeapItem) -> u128

// Adaptive hashing (32/64/128 bits as needed)
