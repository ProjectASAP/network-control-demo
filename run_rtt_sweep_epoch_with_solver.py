...
LEGACY_CSV = Path("rtt_results_epoch_with_solver.csv")
LEGACY_PLOT = Path("query_rtt_plot_epoch_with_solver.png")
LEGACY_TOTAL_PLOT = Path("query_rtt_plot_epoch_with_solver_total.png")
...
def main() -> None:
    ...
    maybe_copy_legacy_output(total_plot, LEGACY_TOTAL_PLOT, args.legacy_root_output)
    if args.server_log != "-":
        maybe_copy_legacy_output(Path(args.server_log), LEGACY_SERVER_LOG, args.legacy_root_output)
    if args.run_solver:
        solver_compare_name = _solver_compare_csv_name(
            args.solver_task_count,
            args.solver_node_count,
            args.query_node_count,
        )
        maybe_copy_legacy_output(
            Path("data") / solver_compare_name,
            Path(solver_compare_name),
            args.legacy_root_output,
        )
